...
function Nuts(opts) {
    ...
    _.bindAll(this, ['_init', 'performQ', 'serveAsset', 'onDownload', 'onPing', 'onUpdateRedirect', 'onUpdate', 'onUpdateWin', 'onServeNotes', 'onServeVersionsFeed', 'onAPIAccessControl']);
            ...
            return that.backend.readAsset(asset)
                .then(function (content) {
                    ...
                    releases = _.chain(releases)
                        ...
                        .map(function (entry) {
                            var gitFilePath = (channel === '*' ? '../../../../' : '../../../../../../');
                            entry.filename = urljoin(fullUrl.replace(url.parse(fullUrl).search, ''), gitFilePath, '/download/' + entry.semver + '/' + entry.filename); //eslint-disable-line node/no-deprecated-api
    ...
    Q()
        ...
        .then(function (versions) {
            ...
            if (latest) {
                ...
                console.log(that.versions.filter());
            }
        })
        .fail(next);
};
